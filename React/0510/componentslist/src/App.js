...
//   const sidebar = (
//     <ul>
//       {props.posts.map((post) => ( //같은 map으로 탄생한 li들은 형제들이며, key값은 이 형제들 사이에서 고유해야한다. 이말은, ul안에서 이 형제들은 id키값 1,2를 갖고, 밑에 const content에서 생성되는 div 2개도 id 1,2를 갖는다. 서로의 키값이 이름이 같읕지언정 각자의 컴포넌트 리스트 안에서만 유일하면 된다. 이러한 key값을 통해 재조정(Reconciliation)에서 어떤 엘리먼트가 변경되어야 하는지 인식함.
