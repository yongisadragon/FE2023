describe("클릭이벤트가 일어나면 데이터를 가져와 화면에 뿌려주는 클래스입니다.", () => {
  ...
  it("click 이벤트가 발생했을때 changeValue 함수가 실행되는지 확인합니다.", () => {
    // 특정 모듈의 함수를 감시하는 자스민 함수
    spyOn(viewManager, "changeValue");
    btnEl.click();
    // toHaveBeenCalled는 함수가 호출된적이 있는지 판별합니다.
    expect(viewManager.changeValue).toHaveBeenCalled();
  });

  it("changeValue 실행되고나서 updateView 함수가 실행되는지 확인합니다.", () => {
    // 특정 모듈의 함수를 감시하는 자스민 함수
